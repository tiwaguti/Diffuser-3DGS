...
from .utils import unwarp_name, points_init, get_random_points, get_random_feauture
...
class PointCloud(BaseModule):
    ...
    def setup(self, point_cloud: Union[SimplePointCloud, None] = None) -> None:
        ...
        if self.cfg.trainable:
            self.position = nn.Parameter(position.contiguous().requires_grad_(True))
            self.features = nn.Parameter(features.contiguous().requires_grad_(True))
    ...
    def re_init(self, num_points) -> None:
        ...
        for atribute in self.atributes:
            ...
            delattr(self, name)
        self.atributes = [
            x for x in self.atributes if x["name"] in ["position", "features"]
        ]
        position = get_random_points(num_points, 1.0)
    ...
    def register_atribute(
        self, name: str, value: Float[Tensor, "3 1"], trainable=True
    ) -> None:
        """
        ...
        if self.cfg.trainable and trainable:
            setattr(self, name, nn.Parameter(value.contiguous().requires_grad_(True)))
        self.atributes.append(
            {
                "name": name,
                "trainable": trainable,
    ...
    def replace(
        self, new_atributes: dict, optimizer: Union[Optimizer, None] = None
    ) -> None:
        """
        ...
        if optimizer is not None:
            replace_tensor = self.replace_optimizer(new_atributes, optimizer)
    ...
    def extand_points(
        ...
        """
        if optimizer is not None:
            extended_tensor = self.extend_optimizer(new_atributes, optimizer)
            ...
            for atribute in self.atributes:
                ...
                extend_atribute = nn.Parameter(
                    torch.cat((value, new_atributes["name"]), dim=0)
                    .contiguous()
                    .requires_grad_(True)
                )
    ...
    def remove_points(
        self, mask: Tensor, optimizer: Union[Optimizer, None] = None
    ) -> None:
        """
        ...
        """
        if optimizer is not None:
            prune_tensor = self.prune_optimizer(mask, optimizer)
            ...
            for atribute in self.atributes:
                ...
                prune_value = nn.Parameter(
                    getattr(self, name)[mask].contiguous().requires_grad_(True)
                )
    ...
    def save_ply(self, path: Path) -> None:
        ...
        for atribute in self.atributes:
            name = atribute["name"]
            if name != "position":
                ply_atribute_list.append(
                    getattr(self, name).reshape(num_points, -1).detach().cpu().numpy()
                )
    ...
    def load_ply(self, path: Path):
        ...
        for attribute in self.atributes:
            ...
            if name != "position":
                value = np.stack(
                    [
                        np.asarray(plydata.elements[0][name + "_{}".format(i)])
                        for i in range(np.prod(shapes))
                    ],
                    axis=1,
                )
                if self.cfg.trainable:
                    setattr(
                        ...
                        name,
                        nn.Parameter(
                            torch.from_numpy(value.reshape(-1, *shapes)).float().cuda()
                        ),
                    )
                else:
