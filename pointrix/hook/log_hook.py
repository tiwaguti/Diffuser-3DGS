...
class LogHook(Hook):
    ...
    def __init__(self):
        ...
        self.losses_test = {"L1_loss": 0.0, "psnr": 0.0, "ssims": 0.0}
    ...
    def before_run(self, trainner) -> None:
        ...
        try:
            Logger.print(
                Panel(Pointrix_logo, title="Welcome to Pointrix", subtitle="Thank you")
            )
            Logger.log("The experiment name is {}".format(trainner.exp_dir))
        except AttributeError:
            Logger.print("ERROR!!..Please provide the exp_name in config file..")
    ...
    def before_train(self, trainner) -> None:
        ...
        self.progress_bar.add_task(
            "train", "Training Progress", trainner.cfg.max_steps, log_dict={}
        )
        self.progress_bar.add_task(
            "validation",
            "Validation Progress",
            len(trainner.datapipeline.validation_dataset),
            log_dict={},
        )
        self.progress_bar.reset("validation", visible=False)
        self.progress_bar.start()
        self.progress_bar.update("train", step=trainner.start_steps)
    ...
    def after_train_iter(self, trainner) -> None:
        ...
        log_dict = {"num_pt": len(trainner.model.point_cloud), "pos_lr": pos_lr}
        log_dict.update(trainner.loss_dict)

        for key, value in log_dict.items():
            if key == "loss":
                self.ema_loss_for_log = 0.4 * value.item() + 0.6 * self.ema_loss_for_log
                self.bar_info.update({key: f"{self.ema_loss_for_log:.{7}f}"})
        ...
        if trainner.global_step % trainner.cfg.bar_upd_interval == 0:
            ...
            self.progress_bar.update(
                "train", step=trainner.cfg.bar_upd_interval, log=self.bar_info
            )
    ...
    def after_val_iter(self, trainner) -> None:
        ...
        if "depth" in trainner.metric_dict:
            visual_depth = visualize_depth(
                trainner.metric_dict["depth"].squeeze(), tensorboard=True
            )
            trainner.logger.write_image(
                "test" + f"_view_{image_name}/depth", visual_depth, step=iteration
            )
        trainner.logger.write_image(
