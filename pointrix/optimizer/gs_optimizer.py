...
class GaussianSplattingOptimizer(BaseOptimizer):
    ...
    def setup(
        self, optimizer: Optimizer, model: BaseModel, cameras_extent: float
    ) -> None:
    ...
    def get_state_dict(self):
        additional_info = {
            ...
            "pos_gradient_accum": self.pos_gradient_accum,
            "denom": self.denom,
        }
        return {**super().state_dict(), **additional_info}

    def load_state_dict(self, state_dict: Mapping[str, Any], append=False):
        max_radii2D = state_dict.pop("max_radii2D")
        pos_gradient_accum = state_dict.pop("pos_gradient_accum")
        denom = state_dict.pop("denom")

        if append:
            self.max_radii2D = torch.cat([self.max_radii2D, max_radii2D])
            self.pos_gradient_accum = torch.cat(
                [self.pos_gradient_accum, pos_gradient_accum]
            )
            self.denom = torch.cat([self.denom, denom])
        else:
    ...
    def update_hypers(self) -> None:
        ...
        self.duplicate_interval = int(C(self.cfg.duplicate_interval, 0, self.step))
        self.opacity_reset_interval = int(
            C(self.cfg.opacity_reset_interval, 0, self.step)
        )
        self.densify_grad_threshold = C(self.cfg.densify_grad_threshold, 0, self.step)
    ...
    def update_structure(
        ...
        if self.step < self.cfg.densify_stop_iter:
            ...
            self.max_radii2D[visibility] = torch.max(
                self.max_radii2D[visibility], radii[visibility]
            )
            self.pos_gradient_accum[visibility] += torch.norm(
                viewspace_grad[visibility, :2], dim=-1, keepdim=True
            )
            ...
            if self.step % self.opacity_reset_interval == 0 or (
                white_bg and self.step == self.cfg.densify_start_iter
            ):
    ...
    def reset_opacity(self) -> None:
        ...
        opacities_new = inverse_sigmoid(torch.min(opc, torch.ones_like(opc) * 0.01))
        self.point_cloud.replace({"opacity": opacities_new}, self.optimizer)
    ...
    def generate_clone_mask(self, grads: Tensor) -> Tensor:
        ...
        mask = torch.where(torch.norm(grads, dim=-1) >= max_grad, True, False)
    ...
    def generate_split_mask(self, grads: Tensor) -> Tensor:
        ...
        mask = torch.logical_and(
            mask, torch.max(scaling, dim=1).values > self.percent_dense * cameras_extent
        )
        return mask
    ...
    def new_pos_scale(self, mask: Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        rots = build_rotation(rotation[mask]).repeat(split_num, 1, 1)
        new_pos = (torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1)) + (
    ...
    def densify_split(self, grads: Tensor) -> None:
        ...
        prune_filter = torch.cat(
            (
                mask,
                torch.zeros(
                    self.split_num * mask.sum(), device=self.device, dtype=bool
                ),
            )
        )
    ...
    def prune(self, step: int) -> None:
        ...
        prune_filter = (self.point_cloud.get_opacity < self.min_opacity).squeeze()
        if size_threshold:
    ...
    def accumulate_viewspace_grad(self, viewspace_points: Tensor) -> Tensor:
        ...
        """
        # Accumulate viewspace gradients for batch
        viewspace_grad = torch.zeros_like(viewspace_points[0])
